...
$res=mysqli_query($con,"select * from coupon where coupon_code='$coupon_str' and status='1'");
...
if($count>0){
	...
	$cart_min_value=$coupon_details['cart_min_value'];
		...
		$jsonArr=array('is_error'=>'yes','result'=>$cart_total,'dd'=>'CartTotal must be Greater than '.$cart_min_value);
		}else{
			if($coupon_type=='TK'){
